...
<td width="26%" align="left" colspan="2">
  <select name="skill" id="skill" style="width:280px;">
    <option value="">Please Choose</option>
<?php 
    $qr_ge = "SELECT * FROM subject_papers WHERE subject = '".substr($_SESSION['userdata']['sub1'],0,3)."HSE"."' ORDER BY paper ASC";
    $rst_ge = mysql_query($qr_ge);
    while($rw_ge = mysql_fetch_assoc($rst_ge)){
?>
<option value="<?php echo $rw_ge['subject'];?>" <?php if(substr($_REQUEST['skill'],0,3) == substr($rw_ge['subject'],0,3)){echo " SELECTED";} ?>><?php echo "[ ".$rw_ge['subject_details']." ] ".$subjects[substr($rw_ge['subject'],0,3)];?></option>
<?php }?>
    </select>
    <?php //echo $subjects[substr($_SESSION['userdata']['sub1'],0,3)];?>
  </td>
 <?php /*<td  width="45%" align="left">
...
<tr>
 <td width="25%" align="left"><strong>SKILL ENHANCEMENT COURSE</strong></td>
 <td width="26%" align="left" colspan="2">
  <?php //echo $subjects[substr($_SESSION['userdata']['main_subject'],0,3)]?>
    <select name="skill" id="skill" style="width:280px;">
    <option value="">Please Choose</option>
<?php 
    $qr_ge = "SELECT * FROM subject_papers WHERE subject = '".substr($_SESSION['userdata']['main_subject'],0,3)."PSE"."' ORDER BY paper ASC";
    $rst_ge = mysql_query($qr_ge);
    while($rw_ge = mysql_fetch_assoc($rst_ge)){
?>
<option value="<?php echo $rw_ge['subject'];?>" <?php if(substr($_REQUEST['skill'],0,3) == substr($rw_ge['subject'],0,3)){echo " SELECTED";} ?>><?php echo "[ ".$rw_ge['subject_details']." ] ".$subjects[substr($rw_ge['subject'],0,3)];?></option>
