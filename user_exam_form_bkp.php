...
<select name="generic" id="generic" style="width:280px;" onchange="getgeneric('generic.php?','generic1');">
<?php
...
<td width="26%" height="30">
 <?php  if(!empty($_REQUEST['generic1'])){
      $str1 .= '<select name="generic1" id="generic1" style="width:350px;" >';
      $str1 .= '<option value="'.$_REQUEST['generic1'].'">'.$_REQUEST['generic1'].'</option>';
      $str1 .='</select>';
      echo $str1;
}else{?>
...
<td width="26%" align="left" colspan="2">
  <select name="skill" id="skill" style="width:280px;">
    <option value="">Please Choose</option>
<?php 
    $qr_ge = "SELECT * FROM `subject_master` WHERE stat = '2E' AND strm IN ('A','B')  AND subject != '".$_SESSION['userdata']['sub1']."' AND fld_value !=".$fld_value[$_SESSION['userdata']['sub1']];
    $rst_ge = mysql_query($qr_ge);
    while($rw_ge = mysql_fetch_assoc($rst_ge)){
?>
