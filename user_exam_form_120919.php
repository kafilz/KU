...
// Unset and assign blank to the session userdata  
...
<body bgcolor="#FFFFFF" <?php  if(!empty($_REQUEST['generic1'])){?> onload="getgeneric('generic.php?','generic1','<?php echo $_REQUEST['generic']?>','<?php echo $_REQUEST['generic1']?>');"<?php }?>>
...
<select name="generic" id="generic" style="width:280px;" onchange="getgeneric('generic.php?','generic1',this.value);">
<?php
if($_SESSION['userdata']['stream']!="BCOM"){?>
  <option value="">Please Choose</option>
<?php if($_SESSION['userdata']['stream']=="BSC"){
  $qr_ge = "SELECT * FROM `subject_master` WHERE stat = '2E' AND strm IN ('S','B')  AND subject != '".$_SESSION['userdata']['sub2']."' AND fld_value !=".$fld_value[$_SESSION['userdata']['main_subject']];
}
else{
  $qr_ge = "SELECT * FROM `subject_master` WHERE stat = '2E' AND strm IN ('A','B')  AND subject != '".$_SESSION['userdata']['sub2']."' AND fld_value !=".$fld_value[$_SESSION['userdata']['main_subject']];
}
...
<td width="26%" height="30">
 <DIV id="generic1" style="float:left;">&nbsp;</DIV>
</td>
</tr>
<tr>
...
<?php 
    echo $qr_ge = "SELECT subject, subject_details FROM subject_master WHERE subject = '".substr($_SESSION['userdata']['sub1'],0,3)."PSE"."' ORDER BY subject ASC"; 
    echo $num_ge = mysql_num_rows($qr_ge);
    if($num_ge > 0){?>
      <option value="">Please Choose</option>

 <?php   while($rw_ge = mysql_fetch_assoc($rst_ge)){
?>
<option value="<?php echo $rw_ge['subject'];?>"><?php echo $rw_ge['subject_details'];?></option>
<?php }}else{?>
...
<table width="90%" border="1" cellspacing="4" cellpadding="5" align="center" style="border-collapse:collapse">
 ...
 <td width="25%" align="left"><strong>MAIN <?php echo $type[$_SESSION['userdata']['is_hons']];?></strong></td>
    <td width="26%" align="left">
<select name="main_subject" id="main_subject" style="width:250px;" onchange="ChangeSkillEnhancement();" >
<?php
...
?>
<option value="<?php echo substr($rw_main['subject'],0,3);?>" <?php if($_REQUEST['main_subject'] == substr($rw_main['subject'],0,3)){echo " SELECTED";} elseif(substr($rw_main['subject'],0,3) == substr($_SESSION['userdata']['main_subject'],0,3)){ echo " SELECTED";}?>>
<?php echo $rw_main['subject_details'];?></option>
<?php }
}
else{
?>
  <option value="<?php echo substr($_SESSION['userdata']['main_subject'],0,3);?>"> <?php echo $subjects[substr($_SESSION['userdata']['main_subject'],0,3)];?></option>
<?php
}
?>
</select>
...
<tr>
 <td width="25%" align="left"><strong>OTH. DISCIPLINE CORE COURSE-2</strong></td>
  <td width="26%" align="left">
<select name="other_subject1" id="other_subject1" style="width:250px;" >
<?php
if($_SESSION['userdata']['imp_pap']!='AP'){
...
?>
<option value="<?php echo substr($rw_subject1['subject'],0,3);?>" <?php if($_REQUEST['other_subject1'] == substr($rw_subject1['subject'],0,3)){echo " SELECTED";} elseif(substr($rw_subject1['sub2'],0,3) == substr($_SESSION['userdata']['main_subject'],0,3)){ echo " SELECTED";}?>>
...
<tr>
 <td width="25%" align="left"><strong>OTH. DISCIPLINE CORE COURSE-3</strong></td>
   <td width="26%" align="left">
<select name="other_subject2" id="other_subject2" style="width:250px;" >
<?php
if($_SESSION['userdata']['imp_pap']!='AP'){
...
<?php 
if($_SESSION['userdata']['imp_pap']=='AP'){
echo $subjects[substr($_SESSION['userdata']['sub1'],0,3)];
}else{?>
<div id="senhancement"><?php echo $subjects[substr($_SESSION['userdata']['sub1'],0,3)];?></div>
<?php }?>
</td> -->
<td width="26%" align="left" colspan="2">
<select name="skill" id="skill" style="width:280px;">
    <option value="">Please Choose</option>
<?php 
    /*$qr_ge = "SELECT * FROM subject_papers WHERE subject = '".substr($_SESSION['userdata']['sub1'],0,3)."PSE"."' ORDER BY paper ASC"; */
    $qr_ge = "SELECT subject, details FROM `subject_papers` WHERE RIGHT(subject,3) = 'PSE' ORDER BY paper ASC";
    $rst_ge = mysql_query($qr_ge);
    while($rw_ge = mysql_fetch_assoc($rst_ge)){
?>
<option value="<?php echo $rw_ge['subject'];?>" <?php if(substr($_REQUEST['skill'],0,3) == substr($rw_ge['subject'],0,3)){echo " SELECTED";} ?>><?php echo $subjects[substr($rw_ge['subject'],0,3)];?></option>
<?php }?>
    </select>
  </td>
<?php
...
<tr>
 <td width="25%" align="left"><strong>OTH. DISCIPLINE CORE COURSE-2</strong></td>
  <td width="26%" align="left">
<select name="other_subject" id="other_subject" style="width:250px;" >
<?php
if($_SESSION['userdata']['imp_pap']!='AP'){
...
<?php 
    ...
    while($rw_ge = mysql_fetch_assoc($rst_ge)){
?>
<option value="<?php echo $rw_ge['subject'];?>" <?php if(substr($_REQUEST['skill'],0,3) == substr($rw_ge['subject'],0,3)){echo " SELECTED";} ?>><?php /*echo $subjects[substr($rw_ge['subject'],0,3)];*/ echo "[ ".$rw_ge['subject']." ] ".$rw_ge['details'];?></option>
<?php }?>
    </select>
  </td>
<?php
